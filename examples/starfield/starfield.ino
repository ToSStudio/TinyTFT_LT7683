...
// This is a demo screen saver mimicing the Win 3.1 starfield

// Helper macro to convert ASCII to numeric
#define CHAR_TO_NUM(c) ((c) - '0')

// ---- Starfield presets (known-good) ----
// Smooth on ATtiny85 @ 8 MHz, I2C Fast Mode (400 kHz)

// Screen + projection
static const int16_t SF_CX = 512;
static const int16_t SF_CY = 300;
static const uint16_t SF_SCALE = 256;

// Motion (subtle)
static const uint8_t SF_Z_STEP = 4;
static const uint8_t SF_Z_MIN = 40;

// Star density (RAM-safe: 3 bytes/star)
static const uint8_t SF_N = 18;  // Number of stars. Try other numbers as desired
static const int8_t SF_R = 120;  // world coord range [-80..+80]

// color constants
static const uint16_t COL_BG = 0x0000;    // black
static const uint16_t COL_STAR = 0xFFFF;  // white


// Size tiers (subtle: only 1x1 and 2x2)
static const uint8_t SF_Z_TIER1 = 170;  // >170 => 1x1
static const uint8_t SF_Z_TIER2 = 90;   // <=170 => 2x2 (and <=90 still 2x2)
...
public:
  ...
  //    writeReg(0x5F, (uint8_t)(x & 0xFF));
  //    writeReg(0x60, (uint8_t)(x >> 8));
  //    writeReg(0x61, (uint8_t)(y & 0xFF));
  //    writeReg(0x62, (uint8_t)(y >> 8));
  //  }
  ...
  //  cmdWrite(0x04);  // start GRAM write
  //  dataWrite2((uint8_t)(clr565 & 0xFF),
  //             (uint8_t)(clr565 >> 8));  // RGB565: low then high
  // }
...
static inline int8_t sfRandXY() {
  int16_t v;
  do {
    v = (int16_t)random(-SF_R, SF_R + 1);
  } while (v == 0);
  return (int8_t)v;
}

static inline uint8_t sfRandZ() {
  // Spawn far away for calmer look
  return (uint8_t)random(120, 256);  // 0..255
}

static inline uint8_t sfSizeFromZ(uint8_t z) {
  if (z <= 90) return 2;
  return 1;
}
...
// static inline uint8_t sfSizeFromZ(uint8_t z) {
//   if (z > SF_Z_TIER1) return 1;
//   if (z > SF_Z_TIER2) return 2;
//   return 2;  // subtle mode: no 3x3/4x4 yet
// }
...
static inline bool sfProject(const Star &s, int16_t &sx, int16_t &sy) {
  // Integer projection: sx = cx + (x*scale)/z
  sx = SF_CX + ((int16_t)s.x * (int16_t)SF_SCALE) / (int16_t)s.z;
  sy = SF_CY + ((int16_t)s.y * (int16_t)SF_SCALE) / (int16_t)s.z;
  return (sx >= 0 && sx < 1024 && sy >= 0 && sy < 600);
}

static inline void sfDrawAt(int16_t sx, int16_t sy, uint8_t size) {
  // size is 1 or 2 here
  uint16_t x1 = (uint16_t)sx;
  uint16_t y1 = (uint16_t)sy;
  uint16_t x2 = x1 + (uint16_t)(size - 1);
  uint16_t y2 = y1 + (uint16_t)(size - 1);
  tft.Fill_rect(x1, y1, x2, y2);
}

static inline void sfRespawn(Star &s) {
  s.x = sfRandXY();
  s.y = sfRandXY();
  s.z = sfRandZ();
}

// Optional: seed RNG from floating ADC on PB3 (do once at boot)
static void sfSeedFromPB3(uint8_t analogPinPB3) {
  uint32_t seed = 0;
  for (uint8_t i = 0; i < 32; i++) {
    seed <<= 1;
    seed ^= (uint32_t)(analogRead(analogPinPB3) & 0x01);
    delay(2);
  }
  randomSeed(seed);
}
...
void starfieldInit(bool clearScreen = true) {
  // If you want ADC-noise seed:
  // sfSeedFromPB3(A3 /* adjust to your PB3 analog pin mapping */);
  ...
  for (uint8_t i = 0; i < SF_N; i++) {
    int16_t sx, sy;
    if (sfProject(stars[i], sx, sy)) {
      uint8_t sz = sfSizeFromZ(stars[i].z);
      sfDrawAt(sx, sy, sz);
    }
  }
}
...
void starfieldStep_pixels() {
  for (uint8_t i = 0; i < SF_N; i++) {
    ...
    // --- 1) ERASE at current position ---
    int16_t sx, sy;
    if (s.z >= SF_Z_MIN && sfProject(s, sx, sy)) {
      uint8_t sz = sfSizeFromZ(s.z);
      if (sz == 1) drawStar1((uint16_t)sx, (uint16_t)sy, COL_BG);
      else drawStar2((uint16_t)sx, (uint16_t)sy, COL_BG);
    }

    // --- 2) UPDATE depth / respawn ---
    if (s.z > (uint8_t)(SF_Z_MIN + SF_Z_STEP)) s.z -= SF_Z_STEP;
    else {
      sfRespawn(s);
      continue;
    }
    ...
    if (s.z >= SF_Z_MIN && sfProject(s, sx, sy)) {
      uint8_t sz = sfSizeFromZ(s.z);
      if (sz == 1) drawStar1((uint16_t)sx, (uint16_t)sy, COL_STAR);
      else drawStar2((uint16_t)sx, (uint16_t)sy, COL_STAR);
    }
  }
}
...
void testPutPixel() {
  // Clear screen (your existing method)
  tft.fore(0, 0, 0);
  tft.Fill_rect(0, 0, 1023, 599);

  // Draw a bright cross at center using putPixel
  for (int16_t dx = -50; dx <= 50; dx++) {
    tft.putPixel(512 + dx, 300, 255, 255, 255);
  }
  for (int16_t dy = -50; dy <= 50; dy++) {
    tft.putPixel(512, 300 + dy, 255, 255, 255);
  }
}

static inline void drawStar1(uint16_t x, uint16_t y, uint16_t c) {
  tft.putPixel565(x, y, c);
}

static inline void drawStar2(uint16_t x, uint16_t y, uint16_t c) {
  // 2x2 block
  tft.putPixel565(x, y, c);
  tft.putPixel565(x + 1, y, c);
  tft.putPixel565(x, y + 1, c);
  tft.putPixel565(x + 1, y + 1, c);
}
...
void setup() {
  ...
  tft.goto_xy(314, 546);
  tft.print(F("ToSStudio - Software"));
  ...
  for (int i = 9; i >= 0; i--) {
    ...
    tft.Fill_rect(460, 160, 660, 450);
    tft.fore(0xFF, 0xFF, 0xFF);
    tft.Plot7Seg(460, 160, i);
    // Serial.print(" .. ");
    // Serial.print(i);
    delay(700);
  }
...
void loop() {
  ...
  const uint32_t frameMs = 30;  // 20 FPS target (try 40ms=25FPS, 66ms=15FPS)
